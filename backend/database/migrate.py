...
from database.db import query, execute, init_db, close_db
...
async def run_migrations():
    ...
    try:
        await init_db()
        for statement in migrations[4]['up'].split(';'):
            if statement.strip():
                await execute(statement)

        for migration in migrations:
            result = await query('SELECT * FROM migrations WHERE name = $1', [migration['name']])
            if len(result.rows) == 0:
                ...
                print(f"Skipping migration (already applied): {migration['name']}")
